...
Four buckets, each keyed on sku — here the answer's permalink:
  ...
  changed        — sku present in both, with a different upvote count, view
  ...
  source_changed — sku present in both with a different count, but also a
                   ...
                   needs most: upvotes, views, shares, comments and the full
                   body come from Quora's inline GraphQL payload and are
...
# Quora lets a question be edited or merged into another. That is a real
# event and there is no other column that would show it.
#
# `text_chars` rather than `text`: an answer's body is up to several thousand
# characters, and a diff that printed two of them per changed row would be
# unreadable. The length moving is the signal that the body did.
TRACKED_FIELDS = ("upvotes", "views", "shares", "comments", "answer_count",
                  "text_chars", "title", "author_credential",
                  "is_machine_answer")

# The subset of TRACKED_FIELDS that only exists on a row the inline payload
# reached, and whose comparability therefore depends on both runs having read
# the same view — see diff_products.
COUNT_FIELDS = ("upvotes", "views", "shares", "comments", "answer_count")
...
def _within_tolerance(before: dict, after: dict, changes: dict,
    ...
    reason is worth stating. Quora's counters are LIVE: a measured answer
    ...
    so a genuine collapse in upvotes is not hidden by a tolerance applied
    field-by-field.
    """
    if tolerance_pct <= 0:
        return False
...
def diff_products(old: List[dict], new: List[dict],
    ...
    for sku in old_by_sku.keys() & new_by_sku.keys():
        ...
        # from Quora inline GraphQL payload, which a question or profile page
...
def _print_summary(result: dict) -> None:
    ...
    for p in result["added"]:
        print(f"  + {p.get('sku')}  {p.get('title')}  "
              f"{p.get('upvotes')} upvote(s) by {p.get('author')}")
...
def _check_comparable(args) -> bool:
    ...
    # A LANGUAGE-SITE MISMATCH, which on this site is entirely possible and
    # is the equivalent of a sibling repo currency check.
    #
    # Quora runs twenty-four language hosts and they are different
    # CATALOGUES: a Spanish question lives on es.quora.com and nowhere else,
    # with its own slug. Diffing one against another would report every row
    # as both added and removed, and none of it would be about Quora.
    #
    # `source` catches the ordinary case; the `language` check catches the
    # rest. A run holding rows from more than one host is NORMAL here and is
    # not refused: a feed links to answers published in Spaces and Quora
    # Session subdomains, and those rows are still each other's.
    languages = {}
    sources = {}
    for label, path in (("--old", args.old), ("--new", args.new)):
        try:
            ...
            continue
        seen = {r.get("language") for r in rows if r.get("language")}
        if len(seen) == 1:
            languages[label] = seen.pop()
        hosts = {r.get("source") for r in rows if r.get("source")}
        if len(hosts) == 1:
            sources[label] = hosts.pop()
    if len(set(languages.values())) > 1:
        problems.append(
            f"the two runs are different language sites ({languages}). "
            f"Quora language hosts are different catalogues with different "
            f"question slugs, so every row here would read as both added and "
            f"removed.")
    if len(set(sources.values())) > 1:
        problems.append(
            f"the two runs are from different hosts ({sources}). They are "
            f"different catalogues, so added and removed would describe the "
            f"host change rather than anything about the answers.")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Diff two quora-scraper JSON outputs by sku.")
    ...
    p.add_argument("--price-tolerance-pct", type=float, default=0.0,
                   ...
                   help="Treat a counter move smaller than PCT%% as a live "
                        ...
                        "the flag has a real use here: Quora's view and "
                        "upvote counts are live, so a monitor watching for an "
