...
scrapers, regardless of what URL was requested (topic feed, question page,
profile, anything) — this is deliberate, not scoped to any one page. If
Quora renders a reCAPTCHA challenge anywhere — sign-in and sign-up are the
usual places — this fires.

**Quora HAS a captcha configured and does not render one to an anonymous
reader.** Measured 2026-09-15 across six good captures: Cloudflare
Turnstile's own loader is in the head of every page it serves
(`challenges.cloudflare.com/turnstile/v0/api.js?render=explicit`), and an
empty `cf-turnstile-response` input sits inside a 0x0 fixed div on every one
of them — wired, mounted, never filled. Neither of those is a block marker
and neither is in `product_parser.BOT_CHALLENGE_MARKERS`; a marker present
on every good page is worse than no marker (§18).

What Quora serves when it refuses is Cloudflare's MANAGED challenge, and a
managed challenge has no widget and no sitekey at all — `cType: 'managed'`,
0 `data-sitekey` attributes, 0 Turnstile iframes on the one measured. So
there is nothing here for a solver to answer, and `page_flow.STATE_POLICY`
marks that state retry-but-do-not-solve. This module is a contingency, not
part of the happy path — a bot manager's configuration can change between
deploys, and a scraper that cannot name what stopped it is much harder to
fix.
...
NARROW: `--solve-captcha when-blocked` is the default and counts answer
...
# insurance: a detection that fires on a page whose answers have
# already rendered guards nothing, which is why the default is
# `when-blocked` and why it counts answer permalinks before it spends.
